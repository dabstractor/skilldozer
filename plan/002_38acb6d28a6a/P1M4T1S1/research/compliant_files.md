...
> Note: the authoritative runtime copy of this report lives at
> `.pi-subagents/artifacts/outputs/ae41b980/plan/002_38acb6d28a6a/P1M4T1S1/research/scout_compliant.txt`.
> This `compliant_files.md` is a mirror requested by the task body; contents are identical.
...
PRD §16 block is illustrative ("everything else is committed") and does not
...
go build -trimpath \
  ...
  -o skilldozer .
```
Matches PRD §12.1 step 3 verbatim (incl. `-trimpath`, `-s -w`, git-describe-with-`|| echo dev` fallback). **PASS.**
```bash
...
Non-blocking observation: `.gitignore` carries 5 extra ignore rules
